...
| POST | `/auth/logout` | 登出，注销令牌 | 基础 | 🔒 |
| GET | `/auth/me` | 当前用户信息与角色 | 基础 | 🔒 |
| GET | `/users` | 用户列表（管理） | 标准 | 🔒 admin |
| PATCH | `/users/{id}/role` | 调整用户角色（管理） | 标准 | 🔒 admin |
...
| GET | `/courses/{id}` | 课程详情 | 基础 | 🔒 成员 |
| PATCH | `/courses/{id}` | 修改课程信息 | 基础 | 🔒 成员 |
| DELETE | `/courses/{id}` | 删除课程（处理级联策略） | 基础 | 🔒 创建者/admin |
| POST | `/courses/{id}/join` | 通过邀请码/课程号加入 | 基础 | 🔒 student |
...
| POST | `/tasks` | 创建任务 | 基础 | 🔒 |
| GET | `/tasks/{id}` | 任务详情 | 基础 | 🔒 本人 |
| PATCH | `/tasks/{id}` | 更新任务（含标记完成） | 基础 | 🔒 本人 |
| DELETE | `/tasks/{id}` | 删除任务 | 基础 | 🔒 本人 |
| GET | `/tasks/upcoming` | 即将截止任务（升序、含剩余时间） | 基础 | 🔒 |
| POST | `/tasks/{id}/split` | AI 拆解：生成子任务建议（不落库） | 特色 | 🔒 本人 |
| POST | `/tasks/batch` | 批量创建（确认拆解结果后落库，含父子关联） | 特色 | 🔒 |
...
| POST | `/materials` | 上传资料（multipart，字段见 3.3） | 标准 | 🔒 成员 |
| DELETE | `/materials/{id}` | 删除资料（同步清理向量索引条目） | 标准 | 🔒 上传者/admin |
| GET | `/materials/{id}/parse-status` | 解析状态查询（排队/处理/可问答/失败+原因） | 标准 | 🔒 成员 |
...
| POST | `/conversations/{id}/messages` | 发送消息：`type=chat` 普通对话（基础）；`type=rag` 附带 `course_id` 走课程资料问答（特色，返回 `citations`） | 基础/特色 | 🔒 本人 |
| DELETE | `/conversations/{id}` | 删除会话 | 基础 | 🔒 本人 |

### 2.6 统计（FR-10、FR-17）

| 方法 | 路径 | 说明 | 版本 | 权限 |
| --- | --- | --- | --- | --- |
| GET | `/stats/overview` | 总任务数/已完成/逾期/完成率，按日期范围过滤 | 标准 | 🔒 |
| GET | `/stats/courses/{course_id}/progress` | 单课程完成率与趋势 | 标准 | 🔒 成员 |
...
### 3.7 统计总览 `GET /api/v1/stats/overview?from=2026-09-01&to=2026-09-30`

```json
// 200 返回
{ "total": 12, "completed": 7, "overdue": 2, "completion_rate": 0.58,
  "by_course": [ { "course_id": 3, "course_name": "操作系统", "total": 5, "completed": 4,
                   "completion_rate": 0.8 } ],
  "trend": [ { "date": "2026-09-08", "completed": 2 }, { "date": "2026-09-09", "completed": 1 } ] }
```
...
| API 分组 | 覆盖需求 | 对应版本门槛 |
| --- | --- | --- |
| 认证、课程、任务、对话/会话 | FR-01/02/03/06/07/09 | 基础版（满足 ≥3 个 HTTP API 要求） |
| 资料上传与解析状态、统计、用户管理 | FR-05/10/11/17 | 标准版（含 API 文档与测试要求） |
| RAG 问答（type=rag、citations）、任务拆解 | FR-12/13/14/15/16 | 特色版 |
